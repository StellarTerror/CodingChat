...
export class Cursor {
    ...
    name: string;
    key: string;
    constructor(y: number, x: number, isActive: boolean, name: string, key: string) {
        this.y = y;
        this.x = x;
...
export class Selection {
    ...
    name: string;
    key: string;
    constructor(y1: number, x1: number, y2: number, x2: number, isActive: boolean, name: string, key: string) {
        this.startY = y1;
        this.startX = x1;
        this.endY = y2;
        this.endX = x2;
    ...
    componentDidMount() {
		if (this.editor_div.current && this.monaco_editor == null) {
            ...
            if (document.getElementById("cursor-style") == null) {
                document.getElementsByTagName("head")[0].innerHTML += `<style id="${this.props.name}-cursor-style"></style>`;
            }
